...
**Row authorisation runs after binding, and it has to.** `QueryPolicy` is consulted by `TaqlTemplate` between `bind` and `run`. It cannot run earlier: literals are lifted out at parse time, so a cached plan holds `Auto(slot=3)` and not `'CH-9021'` — two callers asking about two different clients share one plan. So the plan records *which slots constrain which field* (`Plan.restrictions()`, shape-invariant, computed by `plan/FilterRestrictions`), and `Compiled.restrictions(variables)` resolves those to values per call.
